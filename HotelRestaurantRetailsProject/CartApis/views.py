    ...
    def get(self, request):
        try:
            ...
            Food_pending_orders_serializer = HotelFoodOrderSerializer(
                HotelFoodOrder.objects.filter(order_status=False,total_price__gt = 0), many=True
            )

            Food_approved_orders_serializer = HotelFoodOrderSerializer(
                HotelFoodOrder.objects.filter(order_status=True,total_price__gt = 0), many=True
            )
    ...
    def get(self, request):
        try:
            ...
            Drinks_pending_orders_serializer = HotelDrinksOrderSerializer(
                HotelDrinksOrder.objects.filter(order_status=False,total_price__gt = 0), many=True
            )

            Drinks_approved_orders_serializer = HotelDrinksOrderSerializer(
                HotelDrinksOrder.objects.filter(order_status=True,total_price__gt = 0), many=True
            )
